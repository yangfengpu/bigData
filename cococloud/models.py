...
class KeyVal(models.Model):
    container     = models.ForeignKey(TripleContext, \
                                                                  db_index=True)
    key           = models.CharField(max_length=240,\
                                                           db_index=True)
    valueDescr    = models.CharField(max_length=240, \
                                                                  db_index=True)
...
class EducocoNode(models.Model): # This is the vocabulary
    ...
    uriIdentifier  = models.CharField(max_length=500, \
                                                                unique=True) #full_name
    tag                  = models.ManyToManyField(Tag, \
                                                  related_name='vocabulary_tag', \
                                                  db_index=True)
    def __str__(self): 
...
class EducocoTriplePattern(models.Model):
    name          = models.CharField(max_length = 100)
    description   = models.TextField()
    subject       = models.ForeignKey(EducocoNode, \
                                      related_name='triple_subject',\
                                      db_index=True)
    predicate     = models.ForeignKey(EducocoNode, \
                                      related_name='triple_predicate', \
                                      db_index=True)
    subject       = models.ForeignKey(EducocoNode, \
                                      related_name='triple_object', \
                                      db_index=True)
    context       = models.ForeignKey(TripleContext, \
                                      related_name='triple_context')
    def __str__(self): 
        return self.name
